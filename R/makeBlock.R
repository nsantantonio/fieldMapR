...
makeBlock <- function(l, blockName, pile = TRUE, serpentinePlot = FALSE, startSerp = FALSE) {
	...
	longframe <- data.frame(Trial = c(trials), Line = c(lineNames), Entry = c(entries), plotName = c(plotNames), plotNo = c(plotNos), range = c(ranges), pass = c(passes), plantOrder = 1:prod(dim(plotNos)))

	fieldBlock(Trial = trialsR, Line = lineNamesR, Entry = entriesR, plotName = plotNamesR, plotNo = plotNosR, fill = fill, long = longframe, borderPasses = bpasses[[1]], needStake = stakes, blockName = blockName)
}
